...
function homeRoute(hash = "") {
  const base = appHref("");
  return hash ? `${base.replace(/#.*$/, "")}#${String(hash).replace(/^#/, "")}` : base;
}
function searchRoute(params = "") {
  const u = new URL(appHref("search"), location.origin);
...
function movieRoute(id, extra = {}) {
  const u = new URL(appHref("movie"), location.origin);
  u.searchParams.set("id", String(id));
  Object.entries(extra || {}).forEach(([k, v]) => u.searchParams.set(k, String(v)));
  return u.toString();
}
function tvRoute(id, extra = {}) {
  const u = new URL(appHref("tv"), location.origin);
  u.searchParams.set("id", String(id));
  Object.entries(extra || {}).forEach(([k, v]) => u.searchParams.set(k, String(v)));
  return u.toString();
}
...
// ============================================================
//  PAGE DETECTION (static + SPA routes)
// ============================================================
function detectPage(pathname = location.pathname) {
  const p = String(pathname || "");
  if (p.endsWith("movie.html") || /\/movie\/?$/.test(p)) return "movie";
  if (p.endsWith("tv.html") || /\/tv\/?$/.test(p)) return "tv";
  if (p.endsWith("search.html") || /\/search\/?$/.test(p)) return "search";
  if (/profile\.html$|\/profile\/?$/.test(p)) return "profile";
  if (/theater\.html$|\/theater\/?$/.test(p)) return "theater";
  if (/categories\.html$|\/categories\/?$/.test(p) || /\/catalog\/?$/.test(p)) return "catalog";
...
function initNavbar() {
  const navbar = $("#navbar");
  if (!navbar) return;
  if (navbar.dataset.wired) return;
  navbar.dataset.wired = "1";
  ...
  if (searchToggle && navSearch && searchInput) {
    ...
    let navDdTimer = null;
    async function updateNavSearchDropdown() {
      const q = searchInput.value.trim();
      const existing = document.getElementById("nav-search-dropdown");
      if (!q) {
      ...
      if (!dd) {
        ...
        navSearch.appendChild(dd);
      }
      dd.innerHTML = '<div class="nav-search-dd-loading">Searching…</div>';
      try {
        const data = await tmdb(`/search/multi?query=${encodeURIComponent(q)}&page=1`);
        const items = (data.results || [])
          .filter((i) => i && (i.media_type === "movie" || i.media_type === "tv"))
          .slice(0, 4);
        if (!items.length) {
          dd.innerHTML = '<div class="nav-search-dd-empty">No results.</div>';
          return;
        }
        dd.innerHTML = "";
        items.forEach((item) => {
          ...
          btn.type = "button";
          btn.className = "nav-search-dd-card";
          btn.innerHTML = `
            ${poster ? `<img src="${esc(poster)}" alt="${esc(title)}" loading="lazy" />` : `<div class="nav-search-dd-fallback">${esc(title.slice(0, 1))}</div>`}
            <div class="nav-search-dd-meta">
              <div class="nav-search-dd-title">${esc(title)}</div>
              <div class="nav-search-dd-sub">${type.toUpperCase()} · ${year(item.release_date || item.first_air_date || "")}</div>
            </div>
          `;
          btn.onclick = () => {
            dd.remove();
        ...
        dd.innerHTML = '<div class="nav-search-dd-empty">Search failed.</div>';
      }
    }

    searchInput.addEventListener("input", () => {
      clearTimeout(navDdTimer);
      navDdTimer = setTimeout(() => updateNavSearchDropdown(), 220);
    });

    document.addEventListener("click", (e) => {
      const dd = document.getElementById("nav-search-dropdown");
      if (!dd) return;
      if (e.target === dd || dd.contains(e.target) || e.target === searchInput) return;
      dd.remove();
    });
  }
...
function buildMovieCard(item, type = "movie") {
  ...
  const href     = type === "tv" ? tvRoute(id) : movieRoute(id);
...
(function() {
  ...
  function ensureGlobalUI() {
    ...
    if (!document.getElementById("global-catalog-sheet")) {
      const sheet = document.createElement("section");
      sheet.id = "global-catalog-sheet";
      sheet.className = "global-search-sheet";
      sheet.innerHTML = `
        <div class="global-search-sheet-inner">
  ...
  function initGlobalButtons() {
    ...
    window.__slwu = window.__slwu || {};
    if (window.__slwu._globalButtonsWired) return;
    window.__slwu._globalButtonsWired = true;
    ...
    const applyLayout = (mode) => {
      const m = mode === "netflix" ? "netflix" : "classic";
    ...
    const openSearchSheet = (opts = {}) => {
      ...
      if (opts.prefill && input) input.value = String(opts.prefill);
      if (input) input.focus();
      if (results && !(input && input.value.trim())) {
        results.innerHTML = '<div class="tvremote-empty">Type to search.</div>';
      }
      if (input && input.value.trim()) runGlobalSearch(input.value.trim());
    };
    ...
    const hiddenClose = document.getElementById("global-hidden-close");
    ...
    if (hiddenSheet) hiddenSheet.addEventListener("click", (e) => {
      if (e.target === hiddenSheet) closeAllSheets();
    });
    if (catalogSheet) catalogSheet.addEventListener("click", (e) => {
  ...
  function renderRemoteCards(items, container, opts = {}) {
    ...
    if (!items.length) {
      if (!append) container.innerHTML = '<div class="tvremote-empty">No items found.</div>';
      return;
    }
    items.forEach(item => {
      const type = item.media_type === "tv" || item.first_air_date ? "tv" : "movie";
      const title = item.title || item.name || "Untitled";
      const href = type === "tv" ? tvRoute(item.id) : movieRoute(item.id);
  ...
  function initGlobalSearchDock() {
    window.__slwu = window.__slwu || {};
    if (window.__slwu._globalSearchWired) return;
    window.__slwu._globalSearchWired = true;
    renderSearchMyList();
    const input = document.getElementById("global-search-input");
    if (input) {
  ...
  function initTvRemotePanel() {
    window.__slwu = window.__slwu || {};
    if (window.__slwu._tvRemoteWired) return;
    ...
    if (fsBtn && (isIOS || !canFs)) fsBtn.style.display = "none";
  ...
  function watchNowPlaying() {
    window.__slwu = window.__slwu || {};
    if (window.__slwu._nowPlayingWired) return;
    window.__slwu._nowPlayingWired = true;
  ...
  function initProfilePage(opts = {}) {
    if (!/profile\.html$|\/profile\/?$/.test(location.pathname) && opts.page !== "profile") return;
    const mount = opts.mountId ? document.getElementById(opts.mountId) : null;
    const root = mount || document.body;
    if (!mount) document.body.className = "profile-page";
    root.innerHTML = `
    ...
    function renderDashboard(profileName) {
      ...
      if (exportBtn) exportBtn.onclick = () => {
        const pid = localStorage.getItem(ACTIVE_PROFILE_KEY) || "default";
        const payload = {
          profile: profile || null,
          myList: MyList.get(),
          hidden: HiddenList.get(),
        ...
        a.download = `slwu_${(profile?.name || "profile").replace(/\\s+/g, "_")}_export.json`;
        a.click();
  ...
  function initTheaterPage(opts = {}) {
    if (!/theater\.html$|\/theater\/?$/.test(location.pathname) && opts.page !== "theater") return;
    const mount = opts.mountId ? document.getElementById(opts.mountId) : null;
    const root = mount || document.body;
    if (!mount) document.body.className = "theater-page";
    const now = getNowPlaying();
  ...
  function initCategoriesPage() {
    ...
    if (grid) {
      Object.entries(CATALOG_GENRES).forEach(([label, path]) => {
        ...
        a.href = searchRoute({ category: label.toLowerCase() });
        a.textContent = label;
        grid.appendChild(a);
      });
    }
  ...
  function initOwnerPage(opts = {}) {
    const mount = opts.mountId ? document.getElementById(opts.mountId) : null;
    const root = mount || document.body;
    if (!mount) document.body.className = "owner-page";
  ...
  // SPA-friendly boot: React Router calls this on route changes.
  window.__slwu = window.__slwu || {};
  window.__slwu.boot = (page, opts = {}) => {
    const p = page || detectPage();
    const o = { ...(opts || {}), page: p };
    try { initEnhancements(); } catch (_) {}
    try {
      switch (p) {
        case "home":   initHomePage(); break;
        case "movie":  initMoviePage(); break;
  ...
  document.addEventListener("DOMContentLoaded", () => window.__slwu.boot());
})();
